...
def coordinator_scalability_simulation(stop, forced_lambda=None, slot_index=None, fixed_edge_servers=1):
    """
    Simulazione con:
    - Nodo Edge a N server PARALLELI fissi (= fixed_edge_servers), che servono sia job E sia job C (ritorno dal Cloud).
    - Nodo Coordinator con server PARALLELI SCALABILI (min 1, max cs.COORD_SERVERS_MAX) che servono P1..P4 con priorità a P3/P4.
    - Cloud rimane ∞-server.
    Scalabilità del Coordinator decisa su finestre di 1000s usando UTILIZATION_UPPER/LOWER.
    """
    seed = getSeed()
    reset_arrival_temp()
        ...
        for i in range(cs.EDGE_SERVERS):
            if stats.t.current == edge_completion_times[i]:
                ...
                if completed_type == "E":
                    ...
                    # job C completato all'Edge (ritorno dal Cloud)
                    stats.count_C += 1
