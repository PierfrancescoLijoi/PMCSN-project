...
def edge_coord_scalability_simulation_improved(stop, forced_lambda=None, slot_index=None, fixed_edge_servers=2):
    """
    ...
    def kick_assign_all():
        ...
        # Coordinator
        if stats.queue_coord_high or stats.queue_coord_low:
    ...
    results = {
        ...
        'edge_avg_busy_servers':  stats.area_edge.service / T,    # n° medio core occupati
        'edge_throughput': (stats.index_edge / T),

        'edge_E_avg_delay': (stats.area_E.queue / stats.count_E) if stats.count_E > 0 else 0.0,
        'edge_E_avg_response': ((stats.area_E.queue / stats.count_E) if stats.count_E > 0 else 0.0) + cs.EDGE_SERVICE_E,
        ...
        'cloud_avg_busy_servers': stats.area_cloud.service / T,    # ∞-server: n° medio server occupati
        ...
        'coord_utilization': stats.area_coord.service / T,         # n° medio server occupati
        'coord_throughput': (stats.index_coord / T),

        # Tracce scaling (utili per debug/plot)
